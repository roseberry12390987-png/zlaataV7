...
public final class CheckoutPage extends CheckOutPageObjRepo{
	...
	public void performCheckoutCalculationAndExcelLog(String stage) {
	    ...
	    for (WebElement product : productBlocks) {
	        String productName = product.findElement(By.xpath(".//a[@class='cp_name']")).getText();
	        int quantity = Integer.parseInt(product.findElement(By.xpath(".//div[@class='cp_quantity_wrap']")).getText().trim());

	        int maxQtyAllowed = 2; // could be dynamic based on product type
	        int minQtyAllowed = 1;
	    ...
	    if (skipStage) {
	        ...
	        return; // completely skip calculation for this stage
	    }
	    ...
	    for (WebElement product : productBlocks) {
	        ...
	        System.out.printf(CYAN + "%-25s | Qty: %d | MRP: %.2f | Disc: %.2f | Line MRP: %.2f | Line Disc: %.2f | Saved: %.2f%n" + RESET,
	                productName, quantity, mrp, discounted,
	                mrp * quantity, discounted * quantity, (mrp - discounted) * quantity);
	    }
	    ...
	    double finalAmount = totalDiscounted + giftWrap + express + customization
	            - flatDiscount - coupon - threads;
